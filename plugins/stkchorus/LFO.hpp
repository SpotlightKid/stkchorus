...
/**
   ...
   represent the integer part of the number and the 24 LSB the fractional part
...
public:
    ...
    LFO(float samplerate, const float rate=1.0f, waveform_t waveform=LFO::triangle);
    virtual ~LFO() {}
    ...
    const std::string getWaveformName(long index) {
        return waveNames[index];
    }

    /* @return the waveform's name as a C-string (char *) */
    const char * get_C_WaveformName(long index){
        return waveNames[index].c_str();
    }
